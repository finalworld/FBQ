-- Refresh uncollected system bones every ten hours so the world keeps
-- changing even in quiet areas. Position stays fixed and the type always
-- changes; realtime publishes the row updates to active clients.
...
begin
  ...
  for stale_bone in
    select id,bone_type from public.world_bones
    where active and placement_source='system' and updated_at<=now()-interval '10 hours'
    order by updated_at for update skip locked
  loop
    attempts:=0;
    loop
      replacement:=private.random_bone_type();
      attempts:=attempts+1;
      exit when replacement<>stale_bone.bone_type or attempts>=20;
    end loop;
    if replacement=stale_bone.bone_type then
      replacement:=case when stale_bone.bone_type=0 then 1 else 0 end;
    end if;
    update public.world_bones set bone_type=replacement,generation=generation+1,
      updated_at=now() where id=stale_bone.id;
  end loop;
...
-- Apply the refresh immediately around the most recently active player.
do $refresh_stale_bones$
declare c record;
begin
  ...
  if found then perform private.maintain_world_bones(c.latitude,c.longitude); end if;
end $refresh_stale_bones$;
